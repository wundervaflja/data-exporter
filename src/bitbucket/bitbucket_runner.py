...
oauth_authenticator = OAuth1Authenticator(get_config("bitbucket.client_key"), get_config("bitbucket.client_secret"))
bitbucket = Client(oauth_authenticator)
BATCH_SIZE = 100
ONE_HOUR_SEC = 3600
...
def process_user(username, url):
    ...
    user = None
    try:
        user = User.find_user_by_username(username=username, client=bitbucket)
...
def process_repository(repository, url):
    ...
    repository = Repository.find_repository_by_name_and_owner(repository_name=repo_name, owner=user_name,
                                                              client=bitbucket)
...
def process_commits(repository, url):
    ...
    commits = []
    user_name, repo_name = repository.split("/")
    for commit in Commit.find_commits_in_repository(user_name, repo_name, client=bitbucket):
...
def get_commit_stats(repo, commit_hash):
    ...
    r = oauth_authenticator.session.get(
...
def process_refs(repository, url):
    ...
    refs = Ref.find_refs_in_repository(user_name, repository_name, client=bitbucket)
    for ref in refs:
